...
class BertSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        #self.robust_sum = RobustSumApprox(L=3, norm="L2", epsilon =1e-2, gamma=3.0)
        self.robust_sum = RobustSum(L=3, norm="L2", epsilon =1e-2, gamma=3.0)
    ...
    def forward(
        ...
        ###########
        context_layer = self.robust_sum(attention_probs, value_layer)
...
class RobustSumApprox(nn.Module):
    def __init__(self, L=3, norm="L2", epsilon =1e-2, gamma=2.0, p = 0.1):
        super().__init__()
        self.L=L
        self.norm=norm
        self.epsilon=epsilon
    ...
    def forward(self, A, V):
        ...
        K = int(A.shape[2]*self.p)

        for _ in range(self.L):
            # 计算差值的平方，然后求和并开方
            #dist = ((V[:,:,:K,:].unsqueeze(2) - M.unsqueeze(3))**2).sum(-1).sqrt()
            dist = torch.cdist(V[:,:,:K,:],M)
            #dist = torch.cat([(V[:,:,:K,:]- M[:,:,i].unsqueeze(2).repeat(1,1,K,1)).square().sum(-1).sqrt().unsqueeze(-2) for i in range(M.shape[2])],axis=-2)
            ...
            if self.norm == 'L1':
                ...
                #w = torch.matmul(V[:,:,:K,:].unsqueeze(2).repeat(1,1,A.shape[2],1,1) , M.unsqueeze(-1)).squeeze(-1).clamp_(0) 
                
            elif  self.norm == 'MCP':
                w = 1/(dist + self.epsilon) - 1/self.gamma
                w[w<self.epsilon]=self.epsilon
                
                #w = (torch.matmul(V.unsqueeze(2).repeat(1,1,A.shape[2],1,1) , V.unsqueeze(-1))-1/self.gamma).squeeze(-1).clamp_(0) 
            del dist
